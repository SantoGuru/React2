  ...
  return (
    <>
      <div className="bg-neutral-800 text-white overflow-hidden p-1.5 text-sm">
        ...
        <div>Ethereum: {ethereumPrice !== null ? `${ethereumPrice} USD` : "Loading..."}</div>
          <div>Bitcoin: {bitcoinPrice !== null ? `${bitcoinPrice} USD` : "Loading..."}</div>
          <div>LUNA: {lunaPrice !== null ? `${lunaPrice} USD` : "Loading..."}</div>
          <div>Tether: {tetherPrice !== null ? `${tetherPrice} USD` : "Loading..."}</div>
          <div>Uniswap: {uniswapPrice !== null ? `${uniswapPrice} USD` : "Loading..."}</div>
          <div>Dogecoin: {dogecoinPrice !== null ? `${dogecoinPrice} USD` : "Loading..."}</div>
        </div>
        </div>
      <header className="flex justify-between items-center p-6 bg-black text-white">
        <div className="flex items-center space-x-4 ml-5">
          ...
          <nav className="space-x-4">
            <a href="#" className="hover:underline text-sm ml-7">Home</a>
            <a href="#" className="hover:underline text-sm ml-7">Service</a>
            <a href="#" className="hover:underline text-sm ml-7">Market</a>
            <a href="#" className="hover:underline text-sm ml-7">Community</a>
            <a href="#" className="hover:underline text-sm ml-7">Blog</a>
            <a href="#" className="hover:underline text-sm ml-7">About</a>
          </nav>
        </div>
        <div className="flex space-x-4 items-center">
          <div className="flex items-center space-x-1">
            <select className="bg-transparent text-white border-gray-600 rounded px-2 py-1">
              <option value="en" className="bg-zinc-700">ENG</option>
              <option value="es" className="bg-zinc-700">ESP</option>
              <option value="fr" className="bg-zinc-700">FRA</option>
              <option value="de" className="bg-zinc-700">DEU</option>
            </select>
          </div>
          <button className="px-4 py-2 w-[8rem] h-[2.5rem] bg-purple-600 rounded-full hover:bg-purple-700 text-sm">Get Started</button>
        </div>
      </header>
    </>
  );
